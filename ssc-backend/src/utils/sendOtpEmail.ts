import { Resend } from "resend";

const resend = new Resend(process.env.RESEND_API_KEY);

const buildTemplate = ({ firstName, otp, heading, intro }: { firstName: string; otp: string; heading: string; intro: string }) => `
  <div style="font-family:Inter,Segoe UI,sans-serif;max-width:480px;margin:auto;padding:32px;background:#f8fafc;border-radius:12px">
    <h2 style="color:#0b2545;margin:0 0 8px">${heading}</h2>
    <p style="color:#475569;margin:0 0 24px;line-height:1.5">
      Hi ${firstName || "there"}, ${intro}
      This code expires in <strong>10 minutes</strong>.
    </p>
    <div style="font-size:36px;font-weight:700;letter-spacing:10px;color:#f77f00;text-align:center;padding:20px;background:#fff;border-radius:10px;border:2px dashed #f77f00;margin-bottom:20px">
      ${otp}
    </div>
    <p style="color:#94a3b8;font-size:12px;margin:0">
      If you did not request this email, you can safely ignore it.
    </p>
  </div>
`;

export const sendOtpEmail = async ({
  to,
  firstName,
  otp,
  context = "register",
}: {
  to: string;
  firstName: string;
  otp: string;
  context?: "register" | "resend" | "reset";
}) => {
  if (!process.env.RESEND_API_KEY) throw new Error("RESEND_API_KEY env var is not configured");
  if (!process.env.RESEND_FROM) throw new Error("RESEND_FROM env var is not configured");

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(to)) throw new Error("Invalid recipient email address");
  if (!/^\d{6}$/.test(otp)) throw new Error("Invalid OTP format. Must be 6 digits");

  const isResend = context === "resend";
  const isReset = context === "reset";

  const subject = isReset
    ? "Reset your SSC Pathnirman password"
    : isResend
    ? "Your new SSC Pathnirman OTP"
    : "Verify your SSC Pathnirman email";

  const heading = isReset ? "Password reset code" : isResend ? "New verification code" : "Verify your email";

  const intro = isReset
    ? "here is your password reset OTP. Use it to set a new password."
    : isResend
    ? "here is the new OTP you requested."
    : "thanks for signing up. Use the code below to verify your email and unlock your account.";

  const result = await resend.emails.send({
    from: process.env.RESEND_FROM,
    to,
    subject,
    html: buildTemplate({ firstName, otp, heading, intro }),
  });

  if (result.error) throw new Error(`Failed to send email: ${result.error.message}`);

  return result;
};

export default sendOtpEmail;
