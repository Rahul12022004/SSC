    ...
    <div className="cdBlock">
      <div className="cdBlockHeader" onClick={() => setOpen(o => !o)}>
        ...
        <div className="cdBlockActions">
          {isAdmin && (
            <>
              <button className="cdIconBtn" onClick={e => { e.stopPropagation(); onEdit(block); }} title="Edit">
                <FiEdit2 size={15} />
              </button>
              <button className="cdIconBtn danger" onClick={e => { e.stopPropagation(); onDelete(block._id); }} title="Delete">
                <FiTrash2 size={15} />
              </button>
            </>
          )}
...
// ── BlockModal ────────────────────────────────────────────────────────────────
function BlockModal({ block, courseId, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEdit
        ? `${BASE_URL}/courses/${courseId}/blocks/${block._id}`
        : `${BASE_URL}/courses/${courseId}/blocks`;
...
function CourseDetail() {
  ...
  const [error, setError] = useState("");
  const [blockModal, setBlockModal] = useState(null); // null | { mode: "add" | "edit", block? }
  ...
  const handleBlockDelete = async (blockId) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/courses/${id}/blocks/${blockId}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
      if (data.success) setCourse(prev => ({ ...prev, blocks: prev.blocks.filter(b => b._id !== blockId) }));
      else alert(data.message || "Delete failed");
    } catch { alert("Delete failed"); }
  };

  if (loading) return <div className="cdLoadingPage"><div className="cdSpinner" /></div>;
  if (error) return <div className="cdErrorPage"><p>{error}</p><button onClick={() => navigate("/courses")}>← Back</button></div>;
  ...
  return (
    <div className="cdPage">
      <div className="cdHero">
        <button className="cdBackBtn" onClick={() => navigate("/courses")}>
          <FiArrowLeft size={16} /> Back to Courses
        </button>
        {course.imageUrl && <img src={course.imageUrl} alt={course.name} className="cdHeroImg" />}
